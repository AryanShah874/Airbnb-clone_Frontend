...
function PlacesForm(){
    ...
    const addPhotoByLink=async (event)=>{
        ...
        if(photoLink){
            ...
            const response=await fetch("https://airbnb-clone-backend-8i7m.onrender.com/upload", {
                method: 'POST', 
                body: formData
            });
    ...
    const deletePhoto=async (index)=>{
        const response=await fetch(`https://airbnb-clone-backend-8i7m.onrender.com/deletePhoto/${form.photos[index]}`, {
            method: 'DELETE',
    ...
    const handleSubmit=async(event)=>{
        ...
        if(id){
            //update a place
            const response=await fetch("https://airbnb-clone-backend-8i7m.onrender.com/places", {
                method: 'PUT',
                headers: {
                        ...
                        form.photos.map((link, index)=>{
                            return (
                                <div className="relative h-32" key={index}>
                                    ...
                                    <img src={"https://airbnb-clone-backend-8i7m.onrender.com/"+link} alt={"photo"+index} className="h-full w-full object-cover rounded-2xl"/>
                                </div>
                            )
                        })  
                    }
